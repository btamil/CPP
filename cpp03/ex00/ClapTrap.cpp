...
ClapTrap::ClapTrap(void) : name("Original"), hit_pts(10), engy_pts(10), damage(0)
{
	std::cout << "ClapTrap " << this->name << " Default Constrctor called" << std::endl;
}

ClapTrap::ClapTrap(std::string arg_name) : name(arg_name), hit_pts(10), engy_pts(10), damage(0)
{
...
void ClapTrap::beRepaired(unsigned int amount)
{
	...
	if (this->hit_pts == ClapTrap::defaultHitPoints)
	{
		std::cout << this->name << " is fully operational! No need of repairing it." << std::endl;
		return;
	}
	if (this->hit_pts + amount >= ClapTrap::defaultHitPoints)
	{
		this->hit_pts = ClapTrap::defaultHitPoints;
